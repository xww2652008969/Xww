...
public class Tab
{
    ...
    {
        if (Share.LocalContentId == "18014469510992065") gui.AddTab("dev", dev); //开发者查看信息用的
    ...
    {
        ImGui.TextDisabled(Core.Me.GameObjectId.ToString());
        if (Core.Me.GetCurrTarget() != null) ImGui.TextDisabled(Core.Me.GetCurrTarget().GameObjectId.ToString());

        var net = Vphelp.Nextslot();
        if (net != null) ImGui.TextDisabled(net.GetType().Name);
    ...
    {
        if (ImGui.Button("A")) Core.Me.SetPos(xwwhelp.Getflag(0));
        if (ImGui.Button("B")) Core.Me.SetPos(xwwhelp.Getflag(1));
        if (ImGui.Button("C")) Core.Me.SetPos(xwwhelp.Getflag(2));
        if (ImGui.Button("D")) Core.Me.SetPos(xwwhelp.Getflag(3));
        if (ImGui.Button("1")) Core.Me.SetPos(xwwhelp.Getflag(4));
        if (ImGui.Button("2")) Core.Me.SetPos(xwwhelp.Getflag(5));
        if (ImGui.Button("3")) Core.Me.SetPos(xwwhelp.Getflag(6));
        if (ImGui.Button("4")) Core.Me.SetPos(xwwhelp.Getflag(7));
    }
}