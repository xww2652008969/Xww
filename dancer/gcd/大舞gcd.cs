...
public class 大舞gcd:ISlotResolver
{
    public int Check()
    {
        ...
        if (dangcd.技巧舞步.GetSpell().IsReadyWithCanCast() || Core.Me.HasAura(danbuff.技巧舞步))
        {
            return 1;
        }
        return -1;
    ...
    {
        slot.Add(setspell());
        if (setspell().Id == dangcd.技巧舞步结束)
        {
            slot.Add(danoffgcd.进攻之探戈.GetSpell());
        }
    }
