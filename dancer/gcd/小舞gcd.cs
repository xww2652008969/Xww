...
public class 小舞gcd : ISlotResolver
{
    public int Check()
    {
        ...
        if (Core.Me.HasAura(danbuff.落幕舞预备)) return -1;

        if (dangcd.标准舞步.GetSpell().IsReadyWithCanCast() || Core.Me.HasAura(danbuff.标准舞步)) return 1;
        return -1;
    }
    ...
    public Spell setspell()
    {
        ...
        {
            if (Core.Resolve<JobApi_Dancer>().CompleteSteps == 2) return dangcd.标准舞步结束.GetSpell();
